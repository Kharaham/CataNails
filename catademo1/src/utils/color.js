...
function srgbToLinear(u) {
  ...
  export function averageRGBInROI(ctx, x, y, w, h, highlightCut = 0.92) {
    ...
    if (!isFinite(x) || !isFinite(y) || !isFinite(w) || !isFinite(h)) return fallback;
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      const B = data[i + 2];
      const A = data[i + 3];
      if (A < 250) continue; // ignora transparentes
      const V = Math.max(R, G, B) / 255;
      if (V > highlightCut) continue; // ignora brillos fuertes
      r += R; g += G; b += B; n++;
    }
  ...
  // ===== Clasificadores de piel/cabello =====
  ...
  /** Tono de piel por L* (clara / media / morena / oscura) */
  ...
  /** Tipo de cabello (muy simple, por L* y b*) */
  export function detectHairType(hairLab) {
    const { L, b } = hairLab;
    if (L < 38) return "oscuro";          // castaño oscuro/negro
    if (L > 70 && b > 10) return "rubio"; // rubio/dorado
    if (b > 18) return "cobrizo";         // rojizo/cobrizo
    return "medio";                       // castaño medio/claro
  }
  
  // ===== Paletas según undertone + tono de piel =====
  export function palettesForToneUndertone(undertone, tone) {
    const map = {
      calido: {
        clara: [
          { name: "Durazno & Coral suave", swatches: ["#F7B199", "#F0977A", "#E57E5D"] },
        ...
        morena: [
          ...
          { name: "Naranja quemado", swatches: ["#D4632A", "#B5511F", "#943F17"] },
      ...
      frio: {
        ...
        media: [
          { name: "Fucsia & Magenta", swatches: ["#E1559C", "#CF3C8A", "#B42C76"] },
        ...
        oscura: [
          { name: "Violeta intenso", swatches: ["#7A28D1", "#651FB0", "#52198F"] },
  ...
  // ===== Refinador por contraste piel + cabello =====
  export function refinePalettesByAppearance(palettes, skinLab, hairLab) {
    // Ordena cada lista de swatches: prioriza contraste con piel y cabello
    ...
    return palettes.map(p => {
      const sorted = [...p.swatches].sort((c1, c2) => {
        ...
        const s1 = weightSkin * deltaE76(l1, skinLab) + weightHair * deltaE76(l1, hairLab);
        const s2 = weightSkin * deltaE76(l2, skinLab) + weightHair * deltaE76(l2, hairLab);
        return s2 - s1; // mayor score primero
      });
